...
it.describe("Dataset graphing", function (it) {
    ...
    var DS2 = comb.define(Dataset, {
        instance: {
            getters: {
                columns: function () {
                    return Promise.resolve(["id", "x", "y", "graphId"]);
                }
            }
        }
    });
    ...
    var DS3 = comb.define(Dataset, {
        instance: {
            getters: {
                columns: function () {
                    return Promise.resolve(["id", "name", "x", "y", "linesX"]);
                }
            }
        }
    });
    ...
    it.describe("#graph", function (it) {
        it.should("not modify the current dataset's opts", function () {
            ...
            return ds1.graph(ds2, {x: sql.identifier("id")}).then(function (dsOne) {
        ...
        it.should("accept a simple dataset and pass the table to join", function () {
            return ds1.graph(ds2, {x: sql.identifier("id")}).then(function (ds) {
        ...
        it.should("accept a complex dataset and pass it directly to join", function () {
            return ds1.graph(ds2.filter({x: 1}), {x: sql.identifier("id")}).then(function (ds) {
        ...
        it.should("accept a string table name as the dataset", function () {
            return ds1.graph("lines", {x: sql.identifier("id")}).then(function (ds) {
                assert.equal(ds.sql, 'SELECT points.id, points.x, points.y, lines.id AS lines_id, lines.x AS lines_x, lines.y AS lines_y, lines.graphId FROM points LEFT OUTER JOIN lines ON (lines.x = points.id)');
            });
        });
        ...
        it.should("accept a tableAlias option", function () {
            return ds1.graph("lines", {x: sql.identifier("id")}, {tableAlias: "planes"}).then(function (ds) {
        ...
        it.should("accept a implicitQualifier option", function () {
            return ds1.graph("lines", {x: sql.identifier("id")}, {implicitQualifier: "planes"}).then(function (ds) {
        ...
        it.should("accept a joinType option", function (next) {
            return ds1.graph("lines", {x: sql.identifier("id")}, {joinType: "inner"}).then(function (ds) {
        ...
        it.should("not select any columns from the graphed table if select option is false", function () {
            return ds1.graph("lines", {x: sql.identifier("id")}, {select: false}).then(function (d) {
                return d.graph("graphs", {id: sql.identifier("graphId")}).then(function (ds) {
        ...
        it.should("use the given columns if :select option is used", function () {
            return ds1.graph("lines", {x: sql.identifier("id")}, {select: ["x", "graphId"]}).then(function (d) {
                return d.graph("graphs", {id: sql.identifier("graphId")}).then(function (ds) {
        ...
        it.should("not add columns if graph is called after set_graph_aliases", function () {
            ...
            return ds.graph("lines", {x: sql.identifier("id")}).then(function (ds) {
        ...
        it.should("allow graphing of multiple datasets", function () {
            return ds1.graph(ds2, {x: sql.identifier("id")}).then(function (d) {
                return d.graph(ds3, {id: sql.identifier("graphId")}).then(function (ds) {
        ...
        it.should("allow graphing of the same dataset multiple times", function () {
            return ds1.graph(ds2, {x: sql.identifier("id")}).then(function (d) {
                return d.graph(ds2, {y: sql.identifier("points__id")}, {tableAlias: "graph"}).then(function (ds) {
    ...
    it.describe("#graphEach", function (it) {
        ...
        it.should("split the result set into component tables", function () {
            return Promise.all([
                ds1.graph(ds2, {x: sql.identifier("id")}).then(function (ds) {
                    ...
                    return ds.all().then(function (results) {
                        assert.lengthOf(results, 1);
                        assert.deepEqual(results[0], {points: {id: 1, x: 2, y: 3}, lines: {id: 4, x: 5, y: 6, graphId: 7}});
                    });
                }),
                ds1.graph(ds2, {x: sql.identifier("id")}).then(function (d) {
                    return d.graph(ds3, {id: "graphId"}).then(function (ds) {
                        ds.fetchRows = function (sql, block) {
            ...
            return Promise.all([
                ...
                ds1.graph(ds2, {x: sql.identifier("id")}).then(function (d) {
                    return d.graph(ds3, {id: sql.identifier("graphId")}).then(function (ds) {
                        ...
                        return ds.all(null).then(function (r) {
                            assert.lengthOf(r, 4);
        ...
        it.should("correctly map values when setGraphAliases is used with a third argument for each entry", function () {
            return ds1.graph("lines", {x: sql.identifier("id")}).then(function (ds) {
                ...
                return ds.all(null).then(function (results) {
                    assert.deepEqual(results, [
