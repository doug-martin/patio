...
module.exports = exports = {
    ...
    checkAndGetModel:checkAndGetModel,
    getModel:getModel,
    plugins: plugins,
    fetchTypes: require("./associations/fetchTypes")
};
...
var Model = exports.Model = define([QueryPlugin, Middleware], {
    instance: {
        /**
    ...
    static: {
        ...
        sync: function (cb) {
            ...
            if (!this.synced) {
                ...
                ret = db.schema(tableName).then(function (schema) {
                    if (!self.synced && schema) {
                        self._setSchema(schema);
                        if (supers && supers.length) {
                            return p.all(supers.map(function (sup) {
                                return sup.sync();
                            })).then(function () {
                                self.synced = true;
        ...
        load: function (vals) {
            ...
            if (!this.synced) {
                //sync our model
                ret = this.sync().then(function () {
                    var m = new Self(vals, true);
                    //call the hooks!
                    return m._hook("post", "load").then(function () {
                        return m;
                    });
                });
            } else {
                var m = new Self(vals, true);
                //call the hooks!
                ret = m._hook("post", "load").then(function () {
                    return m;
                });
            }
            return ret;
...
var allModels = [];
/**@ignore*/
function create(name, supers, modelOptions) {
    if (!patio) {
        (patio = require("../index"));
...
function syncModels(cb) {
    return p.nodeify(asyncArray(allModels).forEach(function (model) {
        return model.sync();
    }, 1), cb);
}

function checkAndGetModel(db, name) {
    var ret;
