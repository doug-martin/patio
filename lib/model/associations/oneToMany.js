    ...
    instance: {
        ...
        _postUpdate: function (next, model) {
            ...
            if (model[removeAssociationFlagName]) {
                ...
                p.nodeify(p.all(pl).then(function () {
        ...
        _preRemove: function (next, model) {
            p.nodeify(this.removeAllItems(model), next);
        },
            ...
            if (model.isNew) {
                ...
                ret = model
                    ._checkTransaction(function () {
                        return p.all(items.map(function (item) {
                            return self.addAssociation(item, model, false);
                        }));
                    })
                    .then(function () {
        ...
        removeItems: function (items, model, remove) {
            ...
            if (model.isNew) {
                ...
                var associationLoaded = self.associationLoaded(model);
