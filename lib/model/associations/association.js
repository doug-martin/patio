 ...
 * @param {fetch.EAGER|fetch.LAZY} options.fetchType the fetch type of the model if patio.fetchTypes.Eager is supplied then
 *                                    the associations are automatically filled, if patio.fetchTypes.Lazy is supplied
 *                                    then a promise is returned and is called back with the loaded models
 * @property {Model} model the model associated with this association.
 * @name Association
...
module.exports = Middleware.extend({
    instance: {
        ...
        constructor: function (options, patio, filter) {
            ...
            this.__hooks = {
                before: {add: null, remove: null, "set": null, load: null},
                after: {add: null, remove: null, "set": null, load: null}
            };
            ...
            this.fetchType = options.fetchType || fetchTypes.LAZY;
        },
        ...
        _forceReloadAssociations: function (model) {
            if (!this.readOnly) {
                delete model.__associations[this.name];
                return p.resolve(model[this.name]);
            }
            return p.resolved();
        },
         ...
         */
        isEager: function () {
            return this.fetchType === fetchTypes.EAGER;
        },
        ...
        _filter: function (parent) {
            ...
            if (!ds) {
                ...
                ds.rowCb = function (item) {
                    ...
                    return model._hook("post", "load").then(function () {
                        return model;
                    });
                };
        ...
        fetch: function (parent) {
            ...
            if (this._checkAssociationKey(parent)) {
                var self = this;
                ret = this._filter(parent)[this._fetchMethod]().then(function (result) {
        ...
        _preRemove: function (next, model) {
            if (this.isOwner && !this.isCascading) {
                ...
                p.nodeify(model[this.associatedDatasetName].update(q), next);
            } else {
                next();
            }
        },
        ...
        _getter: function (model) {
            ...
            if (this.associationLoaded(model)) {
                ...
                return this.isEager() ? assoc : p.resolve(assoc);
            } else if (model.isNew) {
                return null;
            } else {
