...
exports.SingleTableInheritance = comb.define({
    instance: {},
    "static": {
 ...
 *      .manyToOne("manager", {key : "managerId", fetchType : patio.fetchTypes.EAGER}));
 ...
 */
exports.ClassTableInheritance = comb.define({
    ...
    instance: {
        ...
        _remove: function () {
            var q = this._getPrimaryKeyQuery(), db = this.db;
            return asyncArray(this._static.__ctiTables.slice().reverse()).forEach(function (table) {
                return db.from(table).filter(q).remove();
            }, 1);
        },
        ...
        _save: function () {
            ...
            if (Self === Self.__ctiBaseModel) {
                ...
                ret = asyncArray(tables)
                    .forEach(function (table, index) {
                        ...
                        return db.from(table).insert(insert).then(function (id) {
        ...
        _update: function () {
            ...
            return Promise
                .all(tables.map(function (table) {
                    ...
                    return comb.isEmpty(update) ? Promise.resolve() : self.db.from(table).filter(q).update(update);
                }))
                .then(function () {
    ...
    static: {
         ...
         * @param {Function} [opts.keyCb] A callback to invoke on on the key returned from the database. This is useful
        ...
        sync: function (cb) {
            ...
            if (this.__configureOpts && !this.__configured) {
                var self = this;
                return nodeify(this._super().then(function () {
                    if (!self.__configured) {
                        var baseModel = self.__ctiBaseModel = self;
                        var key = self.__ctiKey = opts.key || "key";
                        var keyCallback = opts.keyCb || defaultKeyCallback;
                        ...
                        self.dataset.rowCb = function (r) {
                            if (key) {
                                ...
                                if (model !== baseModel) {
                                    ...
                                    return model.dataset.naked().filter(q).one().then(function (vals) {
