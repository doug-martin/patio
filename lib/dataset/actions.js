...
module.exports = define({
    instance: {
        ...
        stream: function (opts) {
            ...
            if ((rowCb = this.rowCb)) {
                ...
                ret._transform = function (data, encoding, done) {
                    Promise.all(rowCb(data)).then(function (data) {
                        ret.push(data);
                        done();
                    }, done);
                };
         ...
         */
        count: function (cb) {
            return nodeify(this.__aggregateDataset().get(sql.COUNT(sql.literal("*")).as("count")).then(function (res) {
         ...
         */
        remove: function (cb) {
            return nodeify(this.executeDui(this.deleteSql), cb);
        },
         ...
         */
        isEmpty: function (cb) {
            return nodeify(this.get(1).then(function (res) {
        ...
        "import": function (columns, values, opts, cb) {
            ...
            if (isInstanceOf(values, Dataset)) {
                ...
                if (!values.length) {
                    ...
                    ret = Promise.reject(new QueryError("Invalid columns in import"));
                }
                ...
                if (sliceSize) {
                    ret = asyncArray(partition(values, sliceSize)).forEach(function (entries, offset) {
                        ...
                        return self.db.transaction(opts, function () {
                            return Promise.all(self.multiInsertSql(columns, entries).map(function (st, index) {
                    ...
                    ret = this.db.transaction(function () {
                        return Promise.all(statements.map(function (st, index) {
                            return self.executeDui(st).then(function (res) {
                                result[index] = res;
                            });
                        }));
            ...
            return nodeify(ret.then(function () {
        ...
        insert: function () {
            ...
            return nodeify(this.executeInsert(this.insertSql.apply(this, args)), cb);
        },
        ...
        insertMultiple: function (array, block, cb) {
            var ret;
            if (block) {
                ret = Promise.all(array.map(function (i) {
        ...
        last: function () {
            if (!this.__opts.order) {
                return Promise.reject(new QueryError("No order specified"));
            }
            var ds = this.reverse();
        ...
        map: function (column, cb) {
            ...
            return nodeify(new Promise(function(resolve, reject) {
                self.forEach().then(function(data) {
                    column && (data = data[isFunction(column) ? "map" : "pluck"](column));
                    resolve(data);
                });
            }), cb);
        },
        ...
        range: function (column, cb) {
            return nodeify(this.__aggregateDataset()
        ...
        singleRecord: function (cb) {
            return nodeify(this.mergeOptions({limit: 1}).all().then(function (r) {
        ...
        singleValue: function (cb) {
            return nodeify(this.naked().ungraphed().singleRecord().then(function (r) {
        ...
        selectHash: function (keyColumn, valueColumn, cb) {
            ...
            return nodeify(this.select.apply(this, args).map(function (r) {
                map[r[k]] = v ? r[v] : r;
            }).then(function () {
                return map;
            }), cb);
        ...
        toHash: function (keyColumn, valueColumn, cb) {
            ...
            return nodeify(this.map(function (r) {
                map[r[k]] = v ? r[v] : r;
            }).then(function () {
                return map;
            }), cb);
        ...
        truncate: function (cb) {
            return nodeify(this.executeDdl(this.truncateSql), cb);
        },
        ...
        update: function (values, cb) {
            return nodeify(this.executeDui(this.updateSql(values)), cb);
        },
