...
module.exports = define({
    instance: {
        ...
        stream: function (opts) {
            ...
            if ((rowCb = this.rowCb)) {
                ...
                ret._transform = function (data, encoding, done) {
                    p.resolve(rowCb(data)).then(function (data) {
                        ret.push(data);
                        done();
                    }, done);
                };
         ...
         *
         * @return {Promise} a promise that is resolved with an array of rows.
         */
        all: function (block, cb) {
            var self = this;
            var ret = asyncArray(this.forEach().then(function (records) {
                return self.postLoad(records);
            }));
            if (block) {
                ret = ret.forEach(block);
            }
            return p.nodeify(ret, cb);
        },
         ...
         *
         * @return {Promise} a promise that is resolved with the average value of the column.
         */
         ...
         * @param {Function} [cb] the callback to invoke when the action is done.
         *
         * @return {Promise} a promise that is resolved with the the number of records in the dataset.
         */
        count: function (cb) {
            return p.nodeify(this.__aggregateDataset().get(sql.COUNT(sql.literal("*")).as("count")).then(function (res) {
         ...
         * @param {Function} [cb] the callback to invoke when the action is done.
         *
         * @return {Promise} a promise resolved with the
         * number of records deleted, but that is adapter dependent.
         */
        remove: function (cb) {
            return p.nodeify(this.executeDui(this.deleteSql), cb);
        },
         ...
         * @param {Function} [cb] the callback to invoke when the action is done.
         *
         * @return {Promise} a promise that is resolved when the action has completed.
         */
        forEach: function (block, inParalell, cb) {
            var rowCb, ret;
            if (isFunction(inParalell)) {
                cb = inParalell;
                inParalell = void 0;
            }
            if (this.__opts.graph) {
                ...
                if (block) {
                    ret = ret.forEach(block,inParalell);
                }
            }
            return p.nodeify(asyncArray(ret), cb);
        },
         ...
         *
         * @return {Promise} a promise that is resolved with a boolean indicating if the table is empty.
         */
        isEmpty: function (cb) {
            return p.nodeify(this.get(1).then(function (res) {
                return isUndefinedOrNull(res) || res.length === 0;
            }), cb);
        },

        __processFields: function () {
        ...
        fetchPromisedRows: function (sql, opts) {
            var self = this;
            return asyncArray(this.execute(sql, opts).then(function (res) {
                return self.__processRows(res.rows, self.__processFields(res.fields));
            }));
        },
         ...
         *
         * @return {Promise} a promise that is resolved with the either the first matching record.
         ...
         * @param {Function} [cb] the callback to invoke when the action is done.
         *
         * @return {Promise} a promise that will be resolved will the value requested.
         */
         ...
         * @param {Function} [cb] the callback to invoke when the action is done.
         *
         * @return {Promise} a promise that is resolved once all records have been inserted.
         */
        "import": function (columns, values, opts, cb) {
            ...
            if (isInstanceOf(values, Dataset)) {
                ...
                if (!values.length) {
                    ...
                    ret = p.reject(new QueryError("Invalid columns in import"));
                }
                ...
                if (sliceSize) {
                    ret = asyncArray(partition(values, sliceSize)).forEach(function (entries, offset) {
                        ...
                        return self.db.transaction(opts, function () {
                            return p.all(self.multiInsertSql(columns, entries).map(function (st, index) {
                                return self.executeDui(st).then(function (res) {
                    ...
                    ret = this.db.transaction(function () {
                        return p.all(statements.map(function (st, index) {
                            return self.executeDui(st).then(function (res) {
                                result[index] = res;
                            });
                        }));
            ...
            return p.nodeify(ret.then(function () {
         ...
         * @param {Function} [cb] the callback to invoke when the action is done.
         *
         * @return {Promise} See {@link patio.Dataset#import} for return functionality.
         */
         ...
         * @param {Function} [cb] the callback to invoke when the action is done.
         *
         * @return {Promise} a promise that is typically resolved with the ID of the inserted row.
         */
        insert: function () {
            var args = argsToArray(arguments);
            var cb = isFunction(args[args.length - 1]) ? args.pop() : null;
            return p.nodeify(this.executeInsert(this.insertSql.apply(this, args)), cb);
        },
         ...
         *
         * @return {Promise} a promise that will be resolved with the id of each item inserted
         *          in the order that was in the array.
         */
        insertMultiple: function (array, block, cb) {
            var ret;
            if (block) {
                ret = p.all(array.map(function (i) {
         ...
         * @param {Function} [cb] a function to be called when the aciton is complete
         *
         * @return {Promise} a promise that will be resolved with the interval between the min and max values
         * of the column.
         */
         ...
         *
         * @return {Promise} a promise that will be resolved with a single object or array depending on the
         * arguments provided.
         */
        last: function () {
            if (!this.__opts.order) {
                return p.reject(new QueryError("No order specified"));
            }
            var ds = this.reverse();
         ...
         * be used.
         * @param {Function} [cb] a function to be called when the action is complete
         *
         * @return {Promise} a promise resolved with the array of mapped values.
         */
         ...
         * @param {Function} [cb] the callback to invoke when the action is done.
         *
         * @return {Promise} a promise that is resolved with the min and max value, as the first
         * and second args respectively.
         */
        range: function (column, cb) {
            return p.nodeify(this.__aggregateDataset()
                .select(sql.min(this.stringToIdentifier(column)).as("min"), sql.max(this.stringToIdentifier(column)).as("max"))
                .first(), cb);
        },
         ...
         *
         * @return {Promise} a promise resolved with a single row from the database that matched the filter.
         */
        one: function () {
         ...
         * @param {Function} [cb] a function to be called when the aciton is complete
         *
         * @return {Promise} a promise resolved with the first record returned from the query.
         */
        singleRecord: function (cb) {
            return p.nodeify(this.mergeOptions({limit: 1}).all().then(function (r) {
         ...
         * @param {Function} [cb] the callback to invoke when the action is done.
         *
         * @return {Promise} a promise that will be resolved with the first value of the first row returned
         * from the dataset.
         *
         */
        singleValue: function (cb) {
            return p.nodeify(this.naked().ungraphed().singleRecord().then(function (r) {
         ...
         * @param {Function} [cb] the callback to invoke when the action is done.
         *
         * @return {Promise} a promise resolved with the sum of the column.
         */
         ...
         *  @param {Function} [cb] the callback to invoke when the action is done.
         *
         *  @return {Promise} a promise that will be resolved with the CSV string of the results of the
         *  query.
         */
        toCsv: function (includeColumnTitles, cb) {
            ...
            var ret = n.columns.then(function (cols) {
                var vals = [];
         ...
         * @param {Function} [cb] the callback to invoke when the action is done.
         *
         * @return {Promise} a promise that is resolved with an array of hashes, that have the keyColumn
        ...
        selectHash: function (keyColumn, valueColumn, cb) {
            ...
            return p.nodeify(this.select.apply(this, args).map(function (r) {
                map[r[k]] = v ? r[v] : r;
            }).then(function () {
                return map;
            }), cb);
         ...
         * If the valueColumn is not given or null, uses the entire hash as the value.
         *
         * @example
         *  // SELECT * FROM table
         ...
         *  @param {Function} [cb] the callback to invoke when the action is done.
         *
         *  @return {Promise} a promise that will be resolved with the resulting hash.
         */
        toHash: function (keyColumn, valueColumn, cb) {
            ...
            var map = {},
                key = this.__hashIdentifierToName(keyColumn),
                value = this.__hashIdentifierToName(valueColumn);

            return p.nodeify(this.forEach(function (record) {
                if (!map[record[key]]) {
                    map[record[key]] = [];
                }

                map[record[key]].push(value ? record[value] : record);
            }).then(function () {
                return map;
            }), cb);
        },
         ...
         * @param {Function} [cb] the callback to invoke when the action is done.
         *
         * @return {Promise} a promise that is resolved once truncation is complete.
         */
        truncate: function (cb) {
            return p.nodeify(this.executeDdl(this.truncateSql), cb);
        },
         ...
         * @param {Function} [cb] the callback to invoke when the action is done.
         *
         * @return {Promise}  a promise that is generally resolved with the
         * number of rows updated, but that is adapter dependent.
         */
        update: function (values, cb) {
            return p.nodeify(this.executeDui(this.updateSql(values)), cb);
        },
        ...
        getters: {
            ...
            columns: function () {
                if (this.__columns) {
                    ...
                    return asyncArray(ds.forEach().then(function () {
