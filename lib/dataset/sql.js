    ...
    instance: {
        ...
        qualifiedColumnName: function (column, table) {
            if (isString(column)) {
                ...
                var columnTable = parts[0], schema, tableAlias;
                column = parts[1];
                if (!columnTable) {
        ...
        complexExpressionSql: function (op, args) {
            ...
            if ((newOp = isOperators[op]) != null) {
                ...
                l = args[0];
                try {
                    return format("(%s %s %s)", isString(l) ? l : this.literal(l), newOp, this.literal(args[1]));
                } catch (e) {
                    throw new QueryError("Unable to do operation " + l + " " + newOp + " " + args[1]);
                }
