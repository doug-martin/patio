...
var IntegerMigrator = define(Migrator, {
    instance: {
        ...
        run: function () {
            ...
            return this._getLatestMigrationVersion().then(function (target) {
                return self._getCurrentMigrationVersion().then(function (current) {
                    if (current !== target) {
                        var direction = self.direction = current < target ? "up" : "down",
                            isUp = direction === "up",
                            version = 0;
        ...
        _getMigrationFiles: function () {
            if (!this._migrationFiles) {
                ...
                return Promise.all(this._migrationFiles);
            }
        },
        ...
        __createOrAlterMigrationTable: function () {
            var c = this.column, table = this.table, db = this.db, ds = this.db.from(table);
    ...
    instance: {
                    ...
                    return asyncArray(migrations).forEach(function (curr) {
                        ...
                        return DB.transaction(function () {
                            ...
                            if (!isFunction(migration[direction])) {
                                ...
                                return (comb.isPromise(dirP) ? dirP : nextP).then(function () {
                                    return (direction === "up" ? ds.insert(query) : ds.filter(query).remove());
                                });
                            }
                        }).then(function () {
        ...
        __getAppliedMigrations: function () {
            if (!this.__appliedMigrations) {
                ...
                return Promise.all(this.__appliedMigrations);
            }
        },
        ...
        __getMigrationFiles: function () {
            ...
            if (!this.__migrationFiles) {
                ...
                return Promise.all(this.__migrationFiles);
            }
        },
