    ...
    instance: {
        ...
        __alterTableSql: function (table, op) {
            ...
            case "setColumnNull":
                ...
                break;
            case "addIndex":
                return Promise.resolve(this.__indexDefinitionSql(table, op));
            case "dropIndex":
                return Promise.resolve(this.__dropIndexSql(table, op));
            case "addConstraint":
            ...
            case "dropConstraint":
                ...
                break;
            default :
                return Promise.reject(new DatabaseError("Invalid alter table operator '" + op.op + "'"));
            }
            return Promise.resolve(format("ALTER TABLE %s %s", this.__quoteSchemaTable(table), alterTableOp));
        },
        ...
        __createTableIndexesFromGenerator: function (name, generator, options) {
            ...
            if (promises.length) {
                ret = Promise.all(promises).then(function (res) {
                    return res;
                }, function (err) {
                    if (!e) {
                        throw err;
