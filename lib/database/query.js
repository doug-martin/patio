    ...
    instance: {
        ...
        __executePromised: function (sql, opts, conn) {
            ...
            if (conn) {
                ...
                ret.then(pHandler, pHandler);
            } else {
                ret = this._getConnection().then(function (conn) {
        ...
        schema: function (table, opts) {
            ...
            var self = this;
            return this.schemaParseTable(tableName, opts).then(function (cols) {
        ...
        tableExists: function (table, cb) {
            return p.nodeify(this.from(table).first().then(function () {
                return true;
            }, function () {
                return false;
            }), cb);
        ...
        __enqueueTransaction: function (opts, cb) {
            ...
            return new p.Promise(function (resolve, reject) {
                var transaction = function () {
                    self.__transaction(null, opts, cb).then(resolve, reject);
                };
                self.__transactionQueue.enqueue(transaction);
            });
        },
            ...
            ["transaction", "execute"].forEach(function (n) {
                ...
                this[n] = function (arg1, arg2) {
                    ...
                    try {
                        if (n === "transaction") {
                            if (comb.isHash(arg1) && arg1.isolated) {
                                ...
                                if (isFunction(arg1)) {
                                    ret = p.resolveOrPromisfyFunction(arg1, this, this);
                                } else {
                                    if (this.supportsSavepoints && arg1.savepoint) {
                                        ...
                                        ret = p.resolveOrPromisfyFunction(arg2, this, this);
                                    }
                                }
                            }
                        } else {
                    ...
                    if (comb.isInstanceOf(ret, stream.Stream)) {
                        ...
                        promises.push((ret = p.resolve(ret)));
                    }
                    return ret;
                };
            }, this);
            try {
                promises.push(p.resolveOrPromisfyFunction(cb, this, this));
            } catch (e) {
                promises.push(p.reject(e));
            }
            if (promises.length === 0) {
                promises.push(p.resolve());
            }
            var self = this;
            var ret = p.all(promises);
            ret.then(
                function () {
        ...
        __getTransactionConnection: function (conn) {
            var self = this;
            return conn ? p.resolve(conn) : this._getConnection().then(function (conn) {
        ...
        __transaction: function (conn, opts, cb) {
            try {
                ...
                return this.__getTransactionConnection(conn).then(function (conn) {
                    return self.__beginTransaction(conn, opts).then(function () {
                        return self.__transactionProxy(cb, conn)
                            .then(function () {
                                return self.__commitTransaction(conn)
                                    .then(
                                    function (res) {
        ...
        __rollback: function (conn, err) {
            var ret = this.__rollbackTransaction(conn, null, err), self = this;
            ret.then(hitch(this, "__finishTransactionAndCheckForMore", conn), hitch(this, "__finishTransactionAndCheckForMore", conn));
            return ret.then(function () {
                if (conn.__transactionDepth <= 1) {
                    self.__transactionError(err);
                } else {
                    throw err;
                }
            });
        ...
        __beginNewTransaction: function (conn, opts) {
            var self = this;
            return this.__logConnectionExecute(conn, this.beginTransactionSql).then(function () {
