            ...
            if (!this.closed) {
                try {
                    ...
                    ret = new p.Promise(function (resolve, reject) {
                        var fields = [], rows = [];
                        var q = self.connection.query(query)
                            .on("error", reject)
                            .on("row", function (row) {
                                rows.push(row);
                            })
...
DS =exports.PostgresDataset = Dataset.extend({
    instance: {
        ...
        insert: function () {
            ...
            if (this.__opts.returning) {
                ...
                var self = this;
                return this.primaryKey(this.__opts.from).then(function (res) {
                    ...
                    if (l) {
                        return dsPromise.then(function (insertRes) {
                            insertRes = self.__processRows(insertRes.rows, self.__processFields(insertRes.fields));
                            if (l === 1) {
        ...
        getters: {
            ...
            columns: function () {
                var ret;
                if (this.__columns) {
                    ret = p.resolve(this.__columns);
                } else {
                    var self = this;
                    ret = this.db.schema(this.firstSourceTable).then(function (schema) {
...
exports.PostgresDatabase = Database.extend({
    instance: {
        ...
        createConnection: function (opts) {
            ...
            conn.on("error", function (err) {
                console.error("Connection from " + self.uri + " errored removing from pool and reconnecting");
            ...
            conn.on("end", function () {
                if (!ret.closed) {
                    console.error("Connection from " + self.uri + " unexpectedly ended");
                    self.pool.removeConnection(ret);
                    ret.closed = true;
                }
            });
        ...
        listenOnce: function (channel, cb, opts) {
            ...
            return new p.Promise(function (resolve, reject) {
                self.listen(channel, function (payload) {
                    //ensure we are not called twice
                    if (!called) {
                        called = true;
                        self.unListen(channel).then(function () {
                            resolve(payload);
                            self = null;
                        }, reject);
                    }
        ...
        indexes: function (table, opts) {
            ...
            table = parts[1];
            return this.serverVersion().then(function (version) {
                var attNums;
                if (version >= 80100) {
        ...
        serverVersion: function () {
            if (!this.__serverVersion) {
                var self = this;
                this.__serverVersion = this.get(identifier("version").sqlFunction).then(function (version) {
        ...
        schemaParseTable: function (tableName, opts) {
            ...
            return this.serverVersion().then(function (serverVersion) {
