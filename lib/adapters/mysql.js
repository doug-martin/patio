...
var Connection = define({
    instance: {
        ...
        query: function (query) {
            ...
            if (!this.closed) {
                try {
                    ret = new p.Promise(function (resolve, reject) {
                        _this.connection.query(query, function (err, rows, fields) {
                            if (err) {
                                reject(err);
                            } else {
                ...
                ret = p.reject(new Error("Connection already closed"));
            }
            return ret;
        }
    }
...
var DS = Dataset.extend({
    ...
    instance: {
        ...
        _insertIgnoreSql: function () {
            return this.__opts.insertIgnore ? " IGNORE" : "";
        },

        //MySQL supports INSERT ... ON DUPLICATE KEY UPDATE
        _insertOnDuplicateKeyUpdateSql: function () {
        ...
        remove: function () {
            return this.executeDui(this.deleteSql).then(function (c) {
                return c.rows.affectedRows;
            });
        },
        ...
        insert: function () {
            return this.executeDui(this.insertSql.apply(this, arguments)).then(function (c) {
                return c.rows.insertId;
            });
        },

        // Replace (update or insert) the matching row.
        replace: function () {
            return this.executeDui(this.replaceSql.apply(this, arguments)).then(function (c) {
                return c.insertId;
            });
        },
        ...
        update: function () {
            return this.executeDui(this.updateSql.apply(this, arguments)).then(function (c) {
...
DB = exports.MySQLDatabase = Database.extend( {
    instance: {
        ...
        serverVersion: function () {
            ...
            if (!this.__serverVersion) {
                var self = this;
                ret = this.get(sql.version().sqlFunction).then(function (version) {
                    var m = version.match(/(\d+)\.(\d+)\.(\d+)/);
                    self._serverVersion = (parseInt(m[1], 10) * 10000) + (parseInt(m[2], 10) * 100) + parseInt(m[3], 10);
                    return self._serverVersion;
                });
            } else {
                ret = p.resolve(this._serverVersion);
            }
            return ret;
        },
        ...
        use: function (dbName) {
            ...
            return this.disconnect().then(function () {
                return self.run("USE " + dbName).then(function () {
        ...
        __alterTableSql: function (table, op) {
            ...
            if (op.op === "addColumn") {
                ...
                if (related) {
                    delete op.table;
                    ret = this._super(arguments).then(function (sql) {
                        op.table = related;
                ...
                ret = this.schema(table).then(function (schema) {
                    var name = op.name;
                    ...
                    return format(
                        ...
                        self.__quoteSchemaTable(table),
                        self.__quoteIdentifier(op.name),
                        self.__columnDefinitionSql(merge(op, opts))
                    );
                });
            } else if (op.op === "dropIndex") {
                ret = p.resolve(format("%s ON %s", this.__dropIndexSql(table, op), this.__quoteSchemaTable(table)));
            } else {
                ret = this._super(arguments);
            }
            return ret;
        ...
        __beginNewTransaction: function (conn, opts) {
            var self = this;
            return this.__setTransactionIsolation(conn, opts).then(function () {
        ...
        __commitTransaction: function (conn, opts) {
            ...
            if (s) {
                return this.__logConnectionExecute(conn, comb("XA END %s").format(this.literal(s))).then(function () {
        ...
        schemaParseTable: function (tableName) {
        ...
        __typeLiteralGenericBoolean: function () {
            return 'tinyint(1)';
        },
